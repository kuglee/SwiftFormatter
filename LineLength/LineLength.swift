...
public struct LineLengthView: View {
  ...
  public init(store: Store<LineLengthViewState, LineLengthViewAction>) { self.store = store }
        ...
        label: {
          TextField(
            ...
            value: Binding(
              ...
              set: { self.store.send(.lineLengthFilledOut($0)) }), formatter: UIntNumberFormatter()
