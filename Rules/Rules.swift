...
public func rulesViewReducer(state: inout RulesViewState, action: RulesViewAction) -> [Effect<
  RulesViewAction
>] {
  switch action {
...
public struct RulesView: View {
  ...
  public var body: some View {
    VStack(alignment: .leading, spacing: .grid(2)) {
      ...
      List {
        ForEach(self.store.value.rules.keys.sorted().enumeratedArray(), id: \.offset) {
          index, key in
          Toggle(
            isOn: Binding(
